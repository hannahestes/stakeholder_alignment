...
class RuleExtractor:
    ...
    def run_and_extract(self, n_runs=50):
        ...
        print(f"\n✅ Extraction complete: {len(self.all_rules)} rule occurrences found\n")
        
        # Apply OR grouping to combine rules with common prefixes
        print("🔗 Grouping rules with OR logic...\n")
        initial_count = len(self.all_rules)
        self.all_rules = self._group_rules_with_or(self.all_rules)
        print(f"✓ After OR grouping: {len(self.all_rules)} unique grouped rules\n"
              f"   (Combined {initial_count - len(self.all_rules)} variant rules into OR statements)\n")
        
        # Rebuild rule_occurrences from grouped rules
        self.rule_occurrences = defaultdict(lambda: {'count': 0, 'wins': [], 'rows': []})
        for rule in self.all_rules:
            rule_key = self._normalize_rule(rule['rule'])
            self.rule_occurrences[rule_key]['count'] += 1
            self.rule_occurrences[rule_key]['wins'].append(rule['win'])
            self.rule_occurrences[rule_key]['rows'].append(rule['rows'])
    
    def _group_rules_with_or(self, all_extracted_rules):
        """Post-process extracted rules to group by common prefixes and combine with OR.
        
        Finds rules that share a common prefix (same AND conditions) and groups the 
        diverging leaf conditions with OR logic.
        ...
        for rule in all_extracted_rules:
            ...
            # Parse conditions
            # Remove outer parens and split by AND
            cleaned = rule_str.replace('(', '').replace(')', '')
            conditions = [c.strip() for c in cleaned.split(' AND ')]
            
            # Try different prefix lengths (longest first to find deepest common ancestor)
            for prefix_len in range(len(conditions), 0, -1):
                prefix_tuple = tuple(conditions[:prefix_len])
                
                if prefix_tuple not in prefix_groups:
                    prefix_groups[prefix_tuple] = []
                
                prefix_groups[prefix_tuple].append(rule)
                break  # Only use the longest matching prefix
        
        # Now combine rules that share prefixes
        grouped_rules_dict = {}
        
        for prefix_tuple, rules_with_prefix in prefix_groups.items():
            # If multiple rules share this exact prefix, they can be grouped
            if len(rules_with_prefix) > 1:
                # Extract the diverging parts (the parts that differ)
                prefix_str = ' AND '.join(f"({c})" for c in prefix_tuple)
                
                # Find what diverges in these rules
                diverging_parts = []
                for rule in rules_with_prefix:
                    ...
                    # Get the part after the common prefix
                    if len(conditions) > len(prefix_tuple):
                        diverging = conditions[len(prefix_tuple):]
                        diverging_parts.append(diverging)
                
                # If all rules have the same number of diverging conditions and differ in just one
                if diverging_parts and all(len(d) == len(diverging_parts[0]) for d in diverging_parts):
                    # Find which position differs
                    num_diverging_conditions = len(diverging_parts[0])
                    
                    for cond_idx in range(num_diverging_conditions):
                        varying_values = set()
                        all_same_except_idx = True
                        
                        for div_parts in diverging_parts:
                            varying_values.add(div_parts[cond_idx])
                            
                            # Check if all other conditions are the same
                            for other_idx in range(num_diverging_conditions):
                                if other_idx != cond_idx:
                                    if div_parts[other_idx] != diverging_parts[0][other_idx]:
                                        all_same_except_idx = False
                        
                        # If only one condition varies, create an OR rule
                        if all_same_except_idx and len(varying_values) > 1:
                            # Extract the attribute name from the varying condition
                            # e.g., "education_Level == College" → "education_Level"
                            first_var = list(varying_values)[0]
                            
                            # Create OR rule: attribute in [val1, val2, ...]
                            or_values = sorted(list(varying_values))
                            or_clause = f"{first_var.split(' ')[0]} in [{', '.join(or_values)}]"
                            
                            # Build full rule
                            non_varying = [diverging_parts[0][i] for i in range(num_diverging_conditions) if i != cond_idx]
                            full_rule = prefix_str
                            if non_varying:
                                full_rule += ' AND ' + ' AND '.join(f"({c})" for c in non_varying)
                            full_rule += f" AND ({or_clause})"
                            
                            # Aggregate metrics
                            avg_win = sum(r['win'] for r in rules_with_prefix) / len(rules_with_prefix)
                            avg_rows = sum(r['rows'] for r in rules_with_prefix) / len(rules_with_prefix)
                            
                            grouped_rules_dict[full_rule] = {
                                'rule': full_rule,
                                'win': avg_win,
                                'rows': avg_rows,
                                'num_variants': len(rules_with_prefix),
                                'run': rules_with_prefix[0]['run'],
                            }
                            break
            else:
                # Single rule with this prefix, keep as-is
                rule = rules_with_prefix[0]
                grouped_rules_dict[rule['rule']] = rule
        
        return list(grouped_rules_dict.values())
...
Knee Point:
  ...
  Rule: {knee_rule['rule'][:40]}..."""
        ...
        # Place info box outside the plot area (to the right)
