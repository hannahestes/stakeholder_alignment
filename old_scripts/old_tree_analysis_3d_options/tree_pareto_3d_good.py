...
class Tree3DAnalyzer:
    ...
    def generate_3d_visualization(self):
        ...
        # Plot off-frontier trees as gray points
        ...
        if off_frontier:
            ax.scatter(
                ...
                c='#4a90e2', s=80, alpha=0.6, label='Off-frontier trees',
                edgecolors='#2e5cba', linewidth=0.5
            )
        
        # Create Pareto frontier surface
        if len(frontier) >= 3:
            import numpy as np
            from matplotlib.colors import Normalize
            from matplotlib.cm import ScalarMappable, RdYlBu_r
            
            # Extract coordinates
            feat_array = np.array([t['num_features'] for t in frontier])
            stab_array = np.array([t['stability'] for t in frontier])
            acc_array = np.array([t['accuracy'] for t in frontier])
            
            # Create triangulated surface
            ax.plot_trisurf(
                feat_array, stab_array, acc_array,
                cmap='RdYlBu_r', alpha=0.7, edgecolor='#333333', linewidth=0.5,
                shade=True
            )
            
            # Color by accuracy for gradient effect
            norm = Normalize(vmin=acc_array.min(), vmax=acc_array.max())
            ax.plot_trisurf(
                feat_array, stab_array, acc_array,
                cmap='RdYlBu_r', norm=norm, alpha=0.8, 
                edgecolor='#222222', linewidth=0.3, shade=True
            )
            
            # Plot frontier points on top of surface
            ax.scatter(
                feat_array, stab_array, acc_array,
                c=acc_array, cmap='RdYlBu_r', s=120, alpha=0.9,
                edgecolors='#000000', linewidth=1.5, zorder=4, label='Frontier points'
            )
        
        elif frontier:
            # If not enough points for surface, fall back to scatter
            ax.scatter(
        ...
        if knee:
            ax.scatter(
                ...
                c='#e74c3c', s=500, marker='*', alpha=1.0, label='Knee (optimal)',
                edgecolors='#c0392b', linewidth=2, zorder=10
            )
            
            # Annotate knee with box
            knee_text = f"OPTIMAL\n{knee['num_features']} features\n{knee['accuracy']:.0f}% accuracy\n{knee['stability']:.0f}% stable"
            ax.text(
                knee['num_features'] + 0.4, knee['stability'] + 8, knee['accuracy'] + 2,
                knee_text,
                fontsize=11, fontweight='bold', color='white',
                bbox=dict(boxstyle='round,pad=0.7', facecolor='#e74c3c', alpha=0.9, edgecolor='#c0392b', linewidth=2)
            )
