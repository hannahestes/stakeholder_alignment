...
class Tree3DAnalyzer:
    ...
    def generate_3d_visualization(self):
        ...
        # Label only top 3-5 frontier points (by accuracy)
        ...
        for i, tree in enumerate(frontier_sorted, 1):
            ...
            ax.text(
                tree['num_features'], tree['stability'], tree['accuracy'] + 5,
                label_text,
        ...
        # Balanced zone: medium of each
        ax.text(4, 55, 70, 'BALANCED ZONE',
               fontsize=12, fontweight='bold', color='#f39c12', alpha=0.6,
               bbox=dict(boxstyle='round', facecolor='#fdebd0', alpha=0.2, edgecolor='#f39c12', linewidth=1))
        ...
        # Info box using figure text
        info_text = f"""ANALYSIS SUMMARY
Trees: {len(self.trees)} | Frontier: {len(frontier)}
Features: {min(f['num_features'] for f in self.trees)}-{max(f['num_features'] for f in self.trees)}
Accuracy: {min(t['accuracy'] for t in self.trees):.0f}%-{max(t['accuracy'] for t in self.trees):.0f}%
Stability: {min(t['stability'] for t in self.trees):.0f}%-{max(t['stability'] for t in self.trees):.0f}%"""
