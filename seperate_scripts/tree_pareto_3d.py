...
class Tree3DAnalyzer:
    ...
    def extract_tree_metrics(self, output, run_num):
        ...
        return {
            ...
            'raw_output': output,  # Store full EZR output
        }
    ...
    def generate_3d_visualization(self, frontier=None):
        ...
        # Plot off-frontier trees (subtle but visible)
        ...
        if off_frontier:
            ax.scatter(
                ...
                c='#999999', s=60, alpha=0.5, label='Off-frontier trees',
        ...
        if len(frontier) >= 3:
            ...
            # Create norm for frontier points even if no surface
            if frontier:
                frontier_acc_vals = np.array([t['accuracy'] for t in frontier])
                norm = Normalize(vmin=frontier_acc_vals.min(), vmax=frontier_acc_vals.max())
            else:
        ...
        # Label all frontier points (by accuracy rank)
        frontier_sorted = sorted(frontier, key=lambda x: x['accuracy'], reverse=True)
        for i, tree in enumerate(frontier_sorted, 1):
            # Position label directly on the point
            ax.text(
                tree['num_features'], tree['stability'], tree['accuracy'],
                str(i),
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
POINT COLORS (by Accuracy):
  [RED] = High Accuracy
  [YEL] = Medium Accuracy
  [GRN] = Low Accuracy

POINT TYPES:
  [GREY CIRCLES] = Off-frontier trees
  [COLORED CIRCLES] = Frontier trees
  [RED STAR] = Knee (optimal balance)"""
        ...
        # Legend removed - using comprehensive visualization key instead
        ...
        # Frontier points table - ALL of them
...
Stab: {min(t['stability'] for t in self.trees):.0f}%-{max(t['stability'] for t in self.trees):.0f}%"""
    ...
    def save_all_trees_json(self):
        """Save all 50 trees to JSON."""
        import json
        ...
        for tree in self.trees:
            ...
            trees_json['trees'].append(tree_data)
        
        json_path = self.output_dir / 'all_trees.json'
        with open(json_path, 'w') as f:
            json.dump(trees_json, f, indent=2)
        print("  ✓ all_trees.json")
    
    def save_frontier_trees_json(self, frontier):
        """Save frontier trees only to JSON."""
        import json
        ...
        frontier_json = {
            'metadata': {
                ...
                'dataset': str(self.dataset_path),
                'stability_metric': 'Feature frequency (%)',
                'accuracy_range': f"{min(t['accuracy'] for t in frontier):.0f}%-{max(t['accuracy'] for t in frontier):.0f}%",
                'stability_range': f"{min(t['stability'] for t in frontier):.0f}%-{max(t['stability'] for t in frontier):.0f}%",
            },
            'frontier_trees': []
        }
        ...
        for i, tree in enumerate(frontier_sorted, 1):
            ...
            frontier_json['frontier_trees'].append(tree_data)
        
        json_path = self.output_dir / 'frontier_trees.json'
        with open(json_path, 'w') as f:
            json.dump(frontier_json, f, indent=2)
        print("  ✓ frontier_trees.json")
    
    def save_outputs(self):
        """Save analysis to CSV files and JSON trees."""
        ...
        # Save all trees as JSON
        self.save_all_trees_json()
        
        # Save frontier trees as JSON
        self.save_frontier_trees_json(frontier)
        
        # 3D visualization
        self.generate_3d_visualization(frontier=frontier)
