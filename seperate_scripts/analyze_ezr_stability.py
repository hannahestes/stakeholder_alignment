...
class EZRAnalyzer:
    ...
    def parse_ezr_output(self, output, run_num):
        ...
        EZR output format:
        [tree structure with node decisions]
        ...
        #features complexity feature1, feature2, ...
        #total_evals final_win_score
        """
        ...
        # Last line should have the final win score
        # Second to last should have features
        
        if len(lines) < 2:
            return None
        
        # Get the LAST line: should be like "73 75" (evals, win_score)
        last_line = lines[-1].strip()
        last_parts = last_line.split()
        
        if len(last_parts) >= 2:
            try:
                # The second number is the win score
                tree_info['win'] = int(last_parts[-1])
            except ValueError:
                pass
        
        # Get second to last line: should be like "19 3 feature1, feature2, ..."
        if len(lines) >= 2:
            feature_line = lines[-2].strip()
            feature_parts = feature_line.split(maxsplit=2)
            
            if len(feature_parts) >= 3:
                try:
                    # First number is something (num nodes?), second is complexity
                    tree_info['complexity'] = int(feature_parts[1])
                    
                    # Rest is features
                    features_str = feature_parts[2]
                    features = [f.strip().rstrip(',') for f in features_str.split(',')]
                    tree_info['features'] = [f for f in features if f]
                except (ValueError, IndexError):
                    pass
        
        # If we still don't have features, try parsing from tree structure
